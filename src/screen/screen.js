...
export default class PixelScreen {
     ...
     * @param {Color} col color for pixel
     * @param {number} layer which layer to draw 
     */
    pixel(v, col, layer) {
        this.pixelXYRGB(v.x, v.y, col.HEX, layer);
    }
     ...
     * @param {number} layer which layer to draw 
     */
    pixelXYHEX(x, y, hex, layer) {
        this.contexts[layer].fillStyle = hex;
    ...
    pixelSprite(x, y, scale, spriteObj, layer) {
        ...
        for (let i = 0; i < spriteObj.length; i++) {
            ...
            this.contexts[layer].fillRect((x + spriteObj[i].x) * scale,( y + spriteObj[i].y) * scale, scale, scale);
        }
    }
}